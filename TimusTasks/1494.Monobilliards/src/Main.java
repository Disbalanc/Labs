...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        int nextToPush = 1;  // следующий шар для закатывания

        for (int i = 0; i < N; i++) {
            int ball = Integer.parseInt(reader.readLine());

            // Закатываем шары, пока не достигнем нужного
            while (nextToPush <= ball) {
                stack.push(nextToPush);
                nextToPush++;
            }

            // Проверяем, можно ли извлечь нужный шар
            if (!stack.isEmpty() && stack.peek() == ball) {
                stack.pop();
            } else {
